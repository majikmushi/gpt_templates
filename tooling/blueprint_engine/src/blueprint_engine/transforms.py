...
def canonical_to_mermaid_class(model: dict[str, Any]) -> tuple[str, str, list[str]]:
    ...
    rel_map = {
        ...
        "communication": "--", "ownership": "*--", "containment": "*--",
    }
...
def canonical_to_uml_class(model: dict[str, Any]) -> tuple[dict[str, Any], str, list[str]]:
    out = {
        ...
        "classes": [
            {"id": el["id"], "name": _label(el), "stereotypes": [el.get("type", "entity")], "attributes": dict(el.get("properties") or {})}
            for el in model.get("elements", [])
        ],
        "relationships": [
            {
                "id": rel["id"], "type": rel["type"], "source": rel["source"], "target": rel["target"],
                "direction": rel.get("direction", "directed"), "source_cardinality": rel.get("source_cardinality"),
                "target_cardinality": rel.get("target_cardinality"), "semantics": list(rel.get("semantics") or []),
...
def canonical_to_json_schema(model: dict[str, Any]) -> tuple[dict[str, Any], str, list[str]]:
    ...
    for el in model.get("elements", []):
        ...
        for key, value in sorted((el.get("properties") or {}).items()):
            if isinstance(value, bool): t = "boolean"
            elif isinstance(value, int): t = "integer"
            elif isinstance(value, float): t = "number"
            elif isinstance(value, list): t = "array"
            elif isinstance(value, dict): t = "object"
            else: t = "string"
            prop_schema[key] = {"type": t}
        defs[el["id"]] = {"type": "object", "title": _label(el), "properties": prop_schema, "additionalProperties": True, "x-blueprint-element-type": el.get("type")}
    ...
    schema = {
        "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": model["id"],
        "title": (model.get("metadata") or {}).get("title", model["id"]), "type": "object",
        "properties": properties, "$defs": defs,
        "x-blueprint-projection": {"source_type": model.get("type"), "dropped_relationship_ids": [r["id"] for r in model.get("relationships", [])]},
    }
...
def canonical_to_xml(model: dict[str, Any]) -> tuple[str, str, list[str]]:
    ...
    for rel in model.get("relationships", []):
        attrs = {"id": rel["id"], "type": str(rel.get("type", "association")), "source": rel["source"], "target": rel["target"], "direction": str(rel.get("direction", "directed"))}
        if rel.get("source_cardinality") is not None: attrs["source_cardinality"] = str(rel["source_cardinality"])
        if rel.get("target_cardinality") is not None: attrs["target_cardinality"] = str(rel["target_cardinality"])
        rnode = SubElement(relationships, "relationship", attrs)
        for semantic in rel.get("semantics", []):
            s = SubElement(rnode, "semantic"); s.text = str(semantic)
    ...
    for container in model.get("containers", []):
        ...
        if container.get("parent"): cattrs["parent"] = container["parent"]
        cnode = SubElement(containers, "container", cattrs)
        for member in container.get("members", []): SubElement(cnode, "member", {"ref": member})
...
def canonical_to_markdown(model: dict[str, Any]) -> tuple[str, str, list[str]]:
    ...
    for el in model.get("elements", []):
        lines.extend([f"### {_label(el)}", "", f"- ID: `{el['id']}`", f"- Type: `{el.get('type', 'entity')}`"])
        props = el.get("properties") or {}
        if props:
            lines.extend(["", "| Property | Value |", "|---|---|"])
            for key, value in sorted(props.items()): lines.append(f"| {key} | {value} |")
        lines.append("")
...
HANDLERS: dict[str, tuple[str, str, Handler]] = {
    "format.mermaid.class": ("transform.canonical-to-mermaid-class", "binding-dependent", canonical_to_mermaid_class),
    "format.plantuml": ("transform.canonical-to-plantuml", "binding-dependent", canonical_to_plantuml),
    ...
    "format.xml": ("adapter.xml.export", "binding-dependent", canonical_to_xml),
